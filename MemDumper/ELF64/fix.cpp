...
static void _regen_section_header(const Elf_Ehdr_Type *pehdr, const char *buffer, size_t len, Elf_Shdr_Type g_shdr[SHDRS])
{
	...
	if (maxLoad > len) {
		...
		printf("warning load size %#X(%u) is bigger than so size %#X(%u), dump maybe incomplete!!!\n", maxLoad, maxLoad, len, len);
		auto n = paddup(maxLoad - len, 0x1000);
		printf("maybe need more %#X(%u) bytes\n", n, n);
